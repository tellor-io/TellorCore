...
library TellorLibrary {
    ...
    */
    function submitMiningSolution(TellorStorage.TellorStorageStruct storage self,string memory _nonce, uint _requestId, uint _value) public {
        ...
        require(uint(sha256(abi.encodePacked(ripemd160(abi.encodePacked(keccak256(abi.encodePacked(self.currentChallenge, msg.sender,_nonce))))))) % self.uintVars[keccak256("difficulty")] == 0, "Challenge information is not saved");
