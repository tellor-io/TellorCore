...
contract('Upgrade Tests', function(accounts) {
  ...
  //  });
   ...
   it("Test upgrade halfway through dispute", async function () {
        ...
        assert(await oracle.getAddressVars(web3.utils.keccak256("tellorContract")) == oldTellor2.address);
        for(var i = 0;i<5;i++){
          res = await web3.eth.sendTransaction({to:oracle.address,from:accounts[i],gas:7000000,data:oracle2.methods['testSubmitMiningSolution(string,uint256,uint256)']("nonce",57,1200).encodeABI()})
        }
        for(var i=0; i<52;i++){
            x = "USD" + i
            apix = api + i
   ...
   it("Test switch partially through with tips added before new block", async function () {
        ...
        assert(data[0][2] == 41, 'T');
        assert(data[1][2] >20000, 'Th');
        assert(data[0][1] == 31, 'The');
        assert(data[1][1] >1000, 'Ther');
   });
 });    