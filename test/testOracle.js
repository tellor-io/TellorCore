  ...
  let utilities;
   ...
   //  });
    it("Test Get MinersbyValue ", async function () {
        ...
        let res = web3.eth.abi.decodeParameters(['uint256[5]','uint256','uint256[5]','uint256','bytes32'],res.logs['1'])
        console.log(res)
        miners = await oracle.getMinersByRequestIdAndTimestamp(1, res[2]);
    ...
    it("Test miner, alternating api request on Q and auto select", async function () {
        ...
        assert(data[0][0] == 30, 'ID on deck should be 30');
        ...
        assert(data[0][0] == 31, 'The');
        assert(data[1][0] >2000, 'Ther');
    });
