...
contract("Dispute Tests", function(accounts) {
  ...
  it("Test multiple dispute to the same miner", async function() {
    ...
    await master.beginDispute(requestId, times[0], 2, { from: accounts[0] });
    await master.beginDispute(requestId, times[1], 2, { from: accounts[1] });
    await master.beginDispute(requestId, times[2], 2, { from: accounts[0] });
    ...
    assert(dispInfo[7][2] == blocks[0].submitted[requestId]);
    assert(dispInfo[2] == true, "Dispute Vote passed");
    voted = await master.didVote(1, accounts[3]);
    assert(voted == true, "account 3 voted");
    voted = await master.didVote(1, accounts[5]);
  ...
  //   assert(dispInfo[7][2] == blocks[0].submitted[requetsId]);
  //   assert(dispInfo[2] == true, "Dispute Vote passed");
